        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex gap-4">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 text-[#8C6422]">
                <AlertTriangle className="h-5 w-5" />
              </div>
              <div>
                <h3 className="font-serif text-lg text-[#1C1B19] dark:text-zinc-100">
                  {status === "Active" ? "Disable User Account?" : "Enable User Account?"}
                </h3>
                <p className="mt-2 text-sm text-[#6B6660] dark:text-zinc-400 leading-relaxed">
                  Are you sure you want to {status === "Active" ? "suspend" : "restore"} the profile of <span className="font-semibold text-[#1C1B19] dark:text-zinc-200">"{userName}"</span>? Disabled users are barred from logging in or submitting admission preferences.
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 flex items-center justify-end gap-3 border-t border-[#E3DFD6] dark:border-zinc-800 pt-4">
              ...
              <button
                ...
                onClick={onConfirm}
                disabled={toggling}
                className={`rounded-lg px-4 py-2 text-sm font-semibold text-white transition-colors disabled:opacity-50 flex items-center gap-1.5 ${
                  status === "Active"
                    ? "bg-[#A23B2E] hover:bg-[#A23B2E]/90"
                    : "bg-[#1F3A5C] hover:bg-[#1F3A5C]/90 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200"
                }`}
              >
                {toggling ? (
                  <>
